...
| `names/` | пулы имён и ников по региону | — (простые списки) |
...
Локализации пока нет. Механизм (ключи строк, файлы перевода — вариант, но не решение)
выбирается вместе с движком, `docs/adr/0000`. До этого текст живёт инлайном в файлах
сущностей.
...
`python tools/validate_content.py` — схемы плюс ссылочная целостность.
