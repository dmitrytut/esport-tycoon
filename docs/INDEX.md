...
| Начать любую работу над механикой | `adr/0009` (петля спек), затем `openspec/specs/` по теме | — |
...
- `openspec/specs/*` — живое поведение системы: что она делает сейчас. Обновляется
  машинно на `/opsx:archive` в ветке реализации, руками не правится
...
- `specs/NNNN-*.md` — старый формат, заготовки 0002–0006. Новые не заводим (`adr/0009`)
...
интерфейс и диалоги на английском, локализация — после выбора движка.
