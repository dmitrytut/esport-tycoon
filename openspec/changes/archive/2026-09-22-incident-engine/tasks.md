...
- [x] 6.2 Archive `incident-engine` through `/opsx:archive incident-engine`, verify `openspec validate --all` passes and the archived `incident-engine`, `week-loop` and `sim-harness` specifications contain the accepted behavior before opening PR-2.
