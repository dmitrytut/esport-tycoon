...
relative position, week-kind counts, interruption counts, contest facts and the season's
encounters.
...
#### Scenario: Encounters are season-owned and continuations are not

- **WHEN** season 2 starts after a season whose marked entries were all settled
- **THEN** season 2 begins with no encounters of its own, while the `contest` and `encounter`
  continuations carry forward so the next opponent and Contest follow the same streams
...
total, recorded contest facts, the encounter held by each marked entry that has one,
season-calendar RNG continuation, and the completed result at the boundary. No one of those
values SHALL be recoverable only from process memory or a system clock.

An encounter SHALL be stored against the identity of exactly one marked calendar entry, and its
content SHALL be owned by the season-contest capability rather than interpreted here: season
code SHALL NOT read an opponent, a Contest result, a reward or a metric while classifying a week,
accepting a fact, evaluating a goal or producing a result.
...
- **THEN** its goal, calendar, facts, encounters, counts, RNG continuation and result are all
  still present without regenerating any of them

#### Scenario: Season code does not read an encounter

- **WHEN** a marked entry holds a settled encounter with an opponent, a Contest result and a
  reward
