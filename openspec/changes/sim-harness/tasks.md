...
- [ ] 3.1 Add `src/run.ts`: build the collective from the seed via `generatePerformer`, plan
      ...
      unexecuted.
- [ ] 3.2 Record a return of control on a week whose reasons include one the scenario's mask
...
- [ ] 3.3 Verify a unit test that a plan placing a member-targeted activity without a member
      of the collective fails before that week is advanced, the way `advance` fails it.
- [ ] 3.4 Verify the equivalence test: a block-long plan advanced by the walk and by
...
- [ ] 4.2 Verify unit tests for the uninterrupted metric, all driven through a real run
      ...
      block length alone.
- [ ] 4.3 Verify a unit test that a week running two money activities reports the week's
...
- [ ] 5.3 Verify the cross-check test: a short scenario advanced directly through the core in
