...
- Render the room in the decided fixed 3/4 top-down view (ADR 0016) with one shared stand-in Spine skeleton that plays a body track and a head track at once, under the same collapsible DOM sheet. Compare scene legibility and planning usability in both sheet positions, not only FPS, and measure two placeholder atlas page sizes. Neither the stand-in skeleton nor its placeholder atlas becomes final art, a texture-resolution or a memory-budget contract for #37.
- Choose a library-based localization mechanism for a later change while keeping #38 English-only. Record a reproducible real-device performance procedure for 15+ stand-in skeleton figures plus the actual DOM panel, without inventing devices or results.
...
ADR 0016 settles the camera, the art technique and the scaling: hand-drawn 2D on Spine skeletons, a fixed 3/4 top-down view, smooth rather than whole-number scaling. This amendment therefore replaces the front/isometric comparison of geometric figures with one 3/4 view of stand-in skeletons, and adds the official Spine runtime to the shell. That runtime may only ship with a Spine licence, so the licence is an external prerequisite of PR-2, like the named physical devices. The five-tab bar in `design/ui.md` waits for the screens behind it and is not part of #38.

The default demo seed originally specified for #38 (`week-shell-38`) materializes a contest at week 0 and blocks the only supported action before any ordinary week executes. This amendment selects a reproducible seed that starts with ordinary weeks and declares the demo's opening organization and season-goal inputs explicitly. It does not add a contest resolution or redefine a career start; the unsupported marked entry still blocks when reached. The real-device profile remains required before PR-2 acceptance.
