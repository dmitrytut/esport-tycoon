...
tests, content, specs. The lockfile check delegates to pnpm and never repairs files; after
a failure, run `pnpm install --fix-lockfile`. The same gate is run by CI and the local
pre-commit hook (enabled once: `pnpm run hooks:install`).
